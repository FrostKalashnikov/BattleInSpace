...
void URuntimeMeshComponent::EnsureHasRuntimeMesh()
{
	...
	{
		SetRuntimeMesh(NewObject<UPtgRuntimeMesh>(this));
	}
}

void URuntimeMeshComponent::SetRuntimeMesh(UPtgRuntimeMesh* NewMesh)
{
...
{
	UPtgRuntimeMesh* Mesh = GetRuntimeMesh();
	if (Mesh)
	{
...
{
	if (UPtgRuntimeMesh* Mesh = GetRuntimeMesh())
	{
