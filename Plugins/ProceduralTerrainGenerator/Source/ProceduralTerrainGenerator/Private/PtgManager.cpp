...
void APtgManager::GenerateTerrainMesh()
{
	...
	if (staticProvider != nullptr)
	{
		...
		for (int32 LODIndex = 1; LODIndex <= NumberOfLODs; LODIndex++)
		{
			...
			staticProvider->CreateSectionFromComponents(LODIndex - 1, procMeshData.SectionIndex, 0, TArray<FVector3f>(procMeshData.Vertices), procMeshData.Triangles, TArray<FVector3f>(procMeshData.Normals), TArray<FVector2f>(procMeshData.UV0), procMeshData.VertexColors, procMeshData.Tangents, ERuntimeMeshUpdateFrequency::Infrequent, procMeshData.bEnableCollision);
